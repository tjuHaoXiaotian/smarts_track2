...
class SocialVehiclePredictor(nn.Module):
    ...
    @torch.no_grad()
    def predict_with_raw_obs(self, raw_obs, device="cuda"):
        ...
        time_wise_data = self.predict(
            torch.from_numpy(space_wise_data).to(device).float(),
            torch.from_numpy(np_obs).to(device).float(),
        )
...
def prepare_SV_predictor_model(model_file_name="SVP_15.pt"):
    model = SocialVehiclePredictor().to("cuda")
    model.load_state_dict(torch.load(
        Path(__file__).parents[1] / model_file_name
    ))
    return model
