...
"""
Assignments Solution Author: Engin Bozkurt, modified by Hao Xiaotian
...
class LocalPlanner:
    ...
    def get_goal_state_set(self, goal_state, ego_state, candidate_lines=None):
    ...
    def plan_paths(self, goal_state_set, target_line_idx):
        ...
        paths = []
        path_validity = []
        new_target_line_idx = target_line_idx
        for idx, goal_state in enumerate(goal_state_set):
            ...
            if np.linalg.norm([path[0][-1] - goal_state[0],
                ...
                if idx < target_line_idx:
                    new_target_line_idx -= 1
                elif idx == target_line_idx:
                    new_target_line_idx = None
            else:
                paths.append(path)
                path_validity.append(True)

        return paths, path_validity, new_target_line_idx
